...
def make_monthly_package(symbol: str, interval: str, year: int, month: int) -> ArchivePackage:
    normalized = symbol.upper().strip()
    if not normalized.endswith("USDT") or not normalized.replace("_", "").isalnum():
        raise ValueError("research archive symbol must be a USD-M USDT symbol")
    ...
    def fetch_month(self, package: ArchivePackage) -> tuple[Candle, ...]:
        ...
        if data_response.status_code != 200 or checksum_response.status_code != 200:
            raise BinanceArchiveError(
                f"archive fetch failed data={data_response.status_code} checksum={checksum_response.status_code}"
            )
