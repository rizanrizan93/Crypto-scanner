...
def _base_series() -> list[Candle]:
    ...
    for i in range(40):
        ...
        rows.append(_candle(i, str(price), str(next_price + Decimal("0.10")), str(price - Decimal("0.10")), str(next_price)))
        price = next_price
    return rows
...
def test_detects_bullish_impulse_retest_without_future_data() -> None:
    ...
    rows.append(_candle(40, str(rows[-1].close), str(prior_high + Decimal("2.50")), str(rows[-1].close - Decimal("0.05")), str(prior_high + Decimal("2.20"))))
    rows.append(_candle(41, str(prior_high + Decimal("2.10")), str(prior_high + Decimal("2.20")), str(prior_high - Decimal("0.05")), str(prior_high + Decimal("0.20"))))

    signal = detect_impulse_retest(tuple(rows), impulse_atr=Decimal("1.0"), retest_tolerance_atr=Decimal("0.50"))
