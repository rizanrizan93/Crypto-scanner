...
def test_only_complete_utc_days_are_kept():
    ...
    rows = tuple(_candle(first, hour, str(100 + hour)) for hour in (0, 4, 8, 12, 16, 20))
    rows += tuple(_candle(second, hour, str(200 + hour)) for hour in (0, 4, 8, 12, 16))
...
def test_relative_strength_selects_only_positive_trending_leaders():
    ...
    candidate = MomentumCandidate("TEST", lookback_days=14, top_k=2, rebalance_days=1, ema_days=20)
...
def test_stress_cost_never_improves_portfolio_return():
    ...
    for symbol, slope in (("AAAUSDT", 1.5), ("BBBUSDT", 1.0), ("BTCUSDT", 0.8)):
        ...
        for offset in range(180):
            ...
            rows.append(DailyBar(start + timedelta(days=offset), price, price + slope * 0.5))
...
def test_calendar_split_and_summary_are_deterministic():
    ...
    candidate = MomentumCandidate("TEST", lookback_days=14, top_k=1, rebalance_days=1, ema_days=20)
